...
linear_with_size <- function(df) {
  lm(score ~ sex + interview_age + t1_neighbor_corr + brain_size, data = df)}
```

```{r}
# Reduced model for partial r^2 analysis 
linear_reduced <- function(df) {
  lm(score ~ sex + t1_neighbor_corr + brain_size, data = df)}
```
...
ggplot(metric_dict, aes(x = metric, y = sum_surviving_ps)) +
  ...
  geom_hline(yintercept = 56, color = "red", size = 0.5) +
